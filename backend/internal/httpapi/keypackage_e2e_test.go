...
func TestKeyPackageAPIEndToEnd(t *testing.T) {
	...
	publish := map[string]any{"key_packages": []map[string]any{
	...
	if response.Code != http.StatusOK || containsKeyMaterial(response.Body.String()) {
		t.Fatalf("availability response = %d %s", response.Code, response.Body.String())
	}
	...
	for range 2 {
		group.Add(1)
		go func() {
			deferred := fixture.claimWithoutTest()
	...
	for result := range responses {
		if result.status != http.StatusOK {
			t.Fatalf("claim status = %d body = %s", result.status, result.body)
		}
		var envelope struct {
		...
		if len(envelope.Data) != 1 {
			t.Fatalf("claim count = %d, want 1: %s", len(envelope.Data), result.body)
		}
		item := envelope.Data[0]
		if item.KeyPackage == "" || item.Signature == "" {
			t.Fatalf("claim missing material: %#v", item)
		}
		if _, err := base64.RawStdEncoding.DecodeString(item.KeyPackage); err != nil {
			t.Fatalf("claim key package is not raw base64: %v", err)
		}
		if _, err := base64.RawStdEncoding.DecodeString(item.Signature); err != nil {
			t.Fatalf("claim signature is not raw base64: %v", err)
		}
	...
	var prohibitedColumns int
	if err := fixture.pool.QueryRow(context.Background(), `SELECT count(*) FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'mls_key_packages' AND column_name IN ('private_key', 'secret', 'plaintext', 'decrypted_text')`).Scan(&prohibitedColumns); err != nil {
		t.Fatalf("check key package columns: %v", err)
	}
	if prohibitedColumns != 0 {
		t.Fatalf("key package table exposes %d prohibited columns", prohibitedColumns)
	}
}
...
func (fixture keyPackageE2EFixture) claimWithoutTest() *httptest.ResponseRecorder {
	request := httptest.NewRequest(http.MethodPost, "/v1/accounts/"+fixture.ownerAccountID+"/mls/key-packages:claim?limit=1", bytes.NewReader(nil))
	request.Header.Set("Authorization", "Bearer "+fixture.requesterToken)
