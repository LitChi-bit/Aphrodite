...
/// Creates and persists a signed MLS Remove proposal for a leaf index.
///
/// The returned bytes are public TLS protocol material. The authenticated
/// proposal is also persisted in native OpenMLS storage for a later Commit.
///
/// # Safety
/// `handle` must be live and `conversation_id` must be a valid UTF-8 C string.
#[no_mangle]
pub unsafe extern "C" fn aphrodite_openmls_propose_remove_member(
    handle: *mut AphroditeOpenMlsHandle,
    conversation_id: *const c_char,
    leaf_index: u32,
) -> AphroditeOpenMlsBuffer {
    catch_buffer(|| {
        ...
        match engine.propose_remove_member(conversation_id, leaf_index) {
...
mod tests {
    ...
    fn creates_signed_remove_proposal_json() {
        ...
        let handle = unsafe { aphrodite_openmls_open(directory.as_ptr()) };
        assert!(!handle.is_null());
        let device = CString::new("device-ffi-proposal").expect("CString");
        let identity = unsafe { aphrodite_openmls_initialize_device(handle, device.as_ptr()) };
        unsafe { aphrodite_openmls_free_buffer(identity) };
        let conversation = CString::new("conversation-ffi-proposal").expect("CString");
        let group = unsafe { aphrodite_openmls_create_group(handle, conversation.as_ptr()) };
        unsafe { aphrodite_openmls_free_buffer(group) };
